...
**Repo** — source code at `~/Code/<repo-name>/`. Has a `.chris/CONTEXT.md` that any agent loads at session start for instant context. One repo can be part of many concurrent projects.
...
│   ├── .chris/CONTEXT.md       ← Chris's own repo context
...
## CONTEXT.md — Repo Briefs

Every repo you work in should have a `.chris/CONTEXT.md`. This is what agents read when they start a session in that repo — no more re-explaining the codebase.
...
`/wf-new` creates this file for new repos. For existing repos, copy from `~/Code/chris/templates/CONTEXT.md` and fill it in.
...
2. Loads the appropriate `.chris/CONTEXT.md` for each involved repo
